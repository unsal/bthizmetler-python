...
class ProjelerApi():
    ...
    def message(self):
        try:
            ...
            _yillar = self.session.query(Projeler.yil).order_by(Projeler.yil.desc()).group_by(Projeler.yil).all()
            for _yil in _yillar:
                _gruplar = self.session.query(Projeler.grup).filter(Projeler.yil==_yil.yil).order_by(Projeler.grup.desc()).group_by(Projeler.grup).all()
